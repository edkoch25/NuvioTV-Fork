...
class AddonSyncService @Inject constructor(
     ...
     *
     * Main F24: formerly `getRemoteAddonUrls` - a getter name hiding a local
     * write. All callers rely on that write (nothing else applies the
     * name/enabled maps), so the apply stays fused here and the name now
     * tells the truth. Note: names/enabled states are only applied when the
     * remote list is non-empty, so an emptied remote does not clear them.
     */
