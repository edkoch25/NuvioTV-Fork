...
The removals are **engine-level, not features**:

- **IAMF** and **MPEG-H** audio decoder components, and an unused UI component — dropped to slim the
  build. No user-facing loss.
...
manager**, and **subtitle add-on support** are intact. All three home-layout implementations
(Classic / Grid / Modern) are also still present in the code — Modern is the default and is what
you'll normally use; the Classic/Grid pickers are the stock upstream first-run selection, not a
separate everyday setting.
